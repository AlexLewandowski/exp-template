...
def filter_results(conditions, configs, results, name_dict = None):
    ...
    unique_indices = grouped_configs.groupby(grouped_configs.columns.tolist()).apply(lambda x: x.index.tolist(), include_groups=False).tolist()
    ...
    filtered_results = [{
        'mean':  pd.concat([results[i] for i in unique_indices[n]]).groupby(level=0).mean(),
        'std': pd.concat([results[i] for i in unique_indices[n]]).groupby(level=0).std(),
...
def filter_sensitivity_results(conditions, sensitivity_parameter, configs, results, name_dict=None):
    filtered_configs = apply_conditions(configs, conditions)
    grouped_configs = filtered_configs.drop(columns=['config_seed'])
    grouped_configs = grouped_configs.drop(columns=['config_name'])
    ...
    filtered_columns = temporary_grouped_configs.columns[temporary_grouped_configs.nunique() > 1].values
    ...
    temporary_unique_indices = temporary_grouped_configs.groupby(temporary_grouped_configs.columns.tolist()).apply(lambda x: x.index.tolist(), include_groups=False).tolist()
    names = [flatten_name(configs.iloc[i[0]], filtered_columns) for i in temporary_unique_indices]
    if name_dict is not None:
        names = [[name_dict[name[0]]] for name in names]
    ...
    for local_unique_indices in temporary_unique_indices:
        ...
        for sensitivity_value in sensitivity_values:
            split_configs = temporary_configs[temporary_configs[sensitivity_parameter] == sensitivity_value]
            split_unique_index = split_configs.groupby(split_configs.columns.tolist()).apply(lambda x: x.index.tolist(), include_groups=False).tolist()
    ...
    filtered_results = [{
        'mean': pd.concat(
            [pd.concat(
                [results[i].apply(lambda x: sensitivity_aggregation(x)).to_frame().T for i in local_unique_indices]
            ).groupby(level=0).mean() for local_unique_indices in unique_indices[n]],
            keys = sensitivity_values, names=[sensitivity_parameter]),
        'std': pd.concat(
            [pd.concat(
                [results[i].apply(lambda x: sensitivity_aggregation(x)).to_frame().T for i in local_unique_indices]
            ).groupby(level=0).std() for local_unique_indices in unique_indices[n]],
            keys = sensitivity_values, names=[sensitivity_parameter]),
...
def plot_filtered_results(filtered_results,
    ...
    for result in filtered_results:
        ...
        ax.fill_between(x, mean - std/n, mean + std/n, alpha=fillalpha, color = my_colors[i])
        i += 1
    ...
    if legend:
        ax.legend(fontsize=legend_fontsize, ncol=legend_ncol, frameon=False,fancybox=False)
